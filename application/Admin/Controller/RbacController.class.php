...
class RbacController extends AdminbaseController {
    ...
    public function index() {
		...
		$role_ids = array(1);
		$role_list = M('RoleUser')->where(array('user_id'=>sp_get_current_admin_id()))->group('role_id')->select();
		foreach ($role_list as $row) {
			$role_ids[]=$row['role_id'];
		}

//		print_r($role_ids);exit;
        $data = $this->role_model->where(array('id'=>array('not in',$role_ids)))->order(array("listorder" => "ASC", "id" => "DESC"))->select();
    ...
    public function authorize() {
       ...
       //角色ID
        ...
        $menu->icon = array('│ ', '├─ ', '└─ ');
        $menu->nbsp = '&nbsp;&nbsp;&nbsp;';
        $all_menus = $this->initMenu();
        $newmenus=array();

		$limits=$this->auth_access_model
			->alias('access')
			->join('__ROLE_USER__ ru on (ru.role_id=access.role_id)')
			->group('access.rule_name')
			->where(array("ru.user_id"=>sp_get_current_admin_id()))->getField("rule_name",true);//获取权限表数据

		$priv_data=$this->auth_access_model->where(array("role_id"=>$roleid))->getField("rule_name",true);//获取权限表数据
        $result = array();
        foreach ($all_menus as $m){
			if($this->_is_checked($m, '', $limits) || sp_get_current_admin_id()==1){
				$newmenus[$m['id']]=$m;
				$result[] = $m;
			}
