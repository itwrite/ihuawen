...
class ProfileController extends MemberbaseController {
	...
	// 编辑用户资料提交
	public function set_accept_month_email_push_status() {
		if(IS_POST){
			$_POST['id']=$this->userid;
			if ($this->users_model->field('id,accept_month_email_push_status')->create()!==false) {
				if ($this->users_model->save()!==false) {
					$this->user=$this->users_model->find($this->userid);
					sp_update_current_user($this->user);
	...
	// 编辑用户资料提交
	public function set_accept_activity_email_push_status() {
		if(IS_POST){
			$_POST['id']=$this->userid;
			if ($this->users_model->field('id,accept_activity_email_push_status')->create()!==false) {
				if ($this->users_model->save()!==false) {
					$this->user=$this->users_model->find($this->userid);
					sp_update_current_user($this->user);
					$this->success("保存成功！",U("user/center/index"));
