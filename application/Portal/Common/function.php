...
function sp_get_nav_by_term_id($term_id){
	$term = sp_get_term($term_id);

	$nav_m = D('Common/Nav');
	$list = $nav_m->select();
	foreach($list as $row){
		if(strpos($row['href'],"{")){//序列 化的数据
			$href=unserialize(stripslashes($row['href']));
			$param = $href['param'];
			if(isset($param['id']) && $param['id']==$term['term_id']){
				$sub_menus = sp_get_submenu($row['parentid']);
				if($sub_menus){
					return $sub_menus[0];
				}
				return $row;
...
function sp_get_term_ids_from_nav_names($names=''){
	$result = array();
	$where = array();
	if(!empty($names)){
		if(is_numeric($names)){
			$where['id'] = array('in',array($names));
		}else{
			$arr = explode(',',$names);
			$where['label'] = array('in',$arr);
		}
	}
	$nav_m = D('Common/Nav');
	$list = $nav_m->where($where)->select();
	foreach($list as $row){
		if(strpos($row['href'],"{")){//序列 化的数据
			$href=unserialize(stripslashes($row['href']));
			$param = $href['param'];
