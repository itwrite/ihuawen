...
class ApiService {
    ...
    public static function posts($tag,$where=array(),$pagesize=0,$pagetpl=''){
    	...
    	$field = (!empty($tag['field']) ? $tag['field'] : '*').",(UNIX_TIMESTAMP(NOW())-UNIX_TIMESTAMP(posts.post_date)) as period,terms.name as term_name";
    	...
    	$join1 = '__TERMS__ as terms on term_relationships.term_id = terms.term_id';
    	$join2= '__USERS__ as users on posts.user_id = users.id';
...
//            $content['_sql'] = $_sql;
            $where['posts.id'] = array('exp',"in($_sql)");
//            $join_area_str = "__POSTS_TAG_RELATIONSHIPS__ posts_tag_relationships on(posts_tag_relationships.post_id = posts.id and posts_tag_relationships.tag_id={$area_tag_id})";
        }
    	...
    	if (empty($pagesize)) {
    	    $posts=$term_relationships_model
                ...
                ->join($join,'right')//主表post
                ->join($join1,'left')
                ->join($join2,'left')
                ->join($join_area_str,'left')
                ->join(M("PostsAuthor")->getTableName(). ' posts_author on posts.author_id = posts_author.id','left')
                ->field($field)
                ->where($where)
                ->order($order)
                ->group($group)
            ...
            $term_relationships_model
                ...
                ->join(M("PostsAuthor")->getTableName(). ' posts_author on posts.author_id = posts_author.id','left')
                ->group($group)
                ->field("posts.id as ppid")
                ->where($where);
           $sub_sql = $term_relationships_model->buildSql();
//            echo "GGGG:".$sub_sql;
            $tmp = $term_relationships_model->query("select count(DISTINCT ppid) AS t_count from {$sub_sql} tem_table");

            $totalsize = intval($tmp[0]['t_count']);
//            echo "GGGG:".$totalsize;
    	    $pagesize = intval($pagesize);
    	    $page_param = C("VAR_PAGE");

            $cur_p = isset($_GET[$page_param])?(int)$_GET[$page_param]:1;
            $cur_p = $cur_p<1?1:$cur_p;

            $offset = ($cur_p-1)* $pagesize;
...
//            $content['_sql'] = $term_relationships_model->getLastSql();
    	    $content['page']=$page->show('default');
    	    $content['total_pages']=$page->getTotalPages(); // 总页数
    	    $content['count']=intval($totalsize);
    	}
