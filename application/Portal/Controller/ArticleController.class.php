...
class ArticleController extends HomebaseController {
    ...
    public function index() {
        ...
        $nav1 = sp_get_nav_by_term_id($term_id);
    	
    	$posts_model=M("Posts");
    	
    	$article=$posts_model
    	->alias("a")
    	->field('a.*,a.id as post_id,c.user_login,c.user_nicename,b.term_id,t.name,t.sublabel,t.icon')
    	->join("__TERM_RELATIONSHIPS__ b ON a.id = b.object_id")
            ->join("__TERMS__ t ON t.term_id = b.term_id")
        ...
        $term_id = intval($article['term_id']);
    	$terms_model= M("Terms");
    	...
    	$join2= '__USERS__ as c on b.user_id = c.id';
        ...
        $author = M('PostsAuthor')->find($author_id);
    ...
    public function magazine_detail(){
        ...
        $article=$posts_model
            ->alias("a")
            ->field('a.*,a.id as post_id,c.user_login,c.user_nicename,b.term_id')
            ->join("__TERM_RELATIONSHIPS__ b ON a.id = b.object_id")
            ->join("__USERS__ c ON a.user_id = c.id")
            ->where(array('a.id'=>$article_id))
            ->find();
...
//        print_r($article);exit;
    ...
    public function add_post(){
        if(IS_POST){
            ...
            $_POST['post']['user_id']=sp_get_current_userid();
            $article=I("post.post");
            $article['smeta']=json_encode($_POST['smeta']);
            $article['post_content']=safe_html(htmlspecialchars_decode($article['post_content']));
            if ($posts_model->field('post_date,user_id,post_content,post_title,post_modified,smeta')->create($article)!==false) {
    ...
    public function edit(){
        ...
        $this->_getTermTree();
        $post=$posts_model->where(array('id'=>$id,'user_id'=>sp_get_current_userid()))->find();
        if(!empty($post)){
    ...
    public function edit_post(){
        if(IS_POST){
            ...
            $article['smeta']=json_encode($_POST['smeta']);
            $article['post_content']=safe_html(htmlspecialchars_decode($article['post_content']));
            if ($posts_model->field('id,user_id,post_content,post_title,post_modified,smeta')->create($article)!==false) {
                $result=$posts_model->where(array('id'=>$article['id'],'user_id'=>sp_get_current_userid()))->save($article);
                if ($result!==false) {
                    $this->success("文章编辑成功！");
                } else {
                    $this->error("文章编辑失败！");
    ...
    public function landing_page(){
        $this->display("Public@:landing_page");
    }
}
