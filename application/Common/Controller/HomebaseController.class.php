...
class HomebaseController extends AppframeController {
	...
	function assign_nav_terms(){
		$subnavs = sp_get_terms('');

		foreach($subnavs as &$row){
			$row['id'] = $row['term_id'];
			$row['label'] = $row['name'];
			$row['href'] = U('portal/list/index',array('id'=>$row['id']));
		}
//		print_r($subnavs);exit;
...
//print_r($current_tmpl_path);exit;
		// 获取当前主题的模版路径
